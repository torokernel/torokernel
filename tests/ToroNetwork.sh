...
if $debug ; then
	...
	# calling qemu
	if [ "$OSTYPE" == "msys" ] ; then
		qemu-system-x86_64 -s -S -L $qemubios -m 256 -hda $appimg -smp 2 -net nic,model=e1000 -net tap,ifname=TAP2 &
	elif [ "$OSTYPE" == "linux-gnu" ] ; then
		iface=`sudo tunctl -b`
		qemu-system-x86_64 -s -S -m 256 -hda $appimg -smp 2 -net nic,model=e1000 -net tap,ifname=$iface &
	fi
